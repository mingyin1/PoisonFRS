...
class FedRecClient(nn.Module):
    ...
    def train_(self, items_emb, reg=0.1, epoch = None):
        ...
        if torch.isnan(self.items_emb_grad[:,0]).any():
            return None, None, None
        return self.train_all, -self.items_emb_grad*args.lr, loss.cpu().item()
    ...
    def eval_(self, items_emb):
        ...
        if self._target_:
            ...
            er_at_50 = evaluate_recall(rating, self._target_, 50)
            ndcg_at_10 = evaluate_ndcg(rating, self._target_, 10)
            target_result = np.array([er_at_5, er_at_10, ndcg_at_10, er_at_20, er_at_50])
        else:
            target_result = None
